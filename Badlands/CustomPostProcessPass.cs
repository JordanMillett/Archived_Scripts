...
public class CustomPostProcessPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        //---Custom effect here---
        var visionEffect = stack.GetComponent<VisionEffectComponent>();
        // Only process if the effect is active
        if (visionEffect.IsActive())
        {
            var material = materials.VisionEffect;
            // P.s. optimize by caching the property ID somewhere else
            material.SetFloat(Shader.PropertyToID("_Intensity"), visionEffect.intensity.value);
            //material.SetColor(Shader.PropertyToID("_OverlayColor"), visionEffect.overlayColor.value);

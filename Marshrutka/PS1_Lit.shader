Shader "Custom/PS1_Lit"
{
    Properties
    {
        [MainColor] _BaseColor("Color", Color) = (0.5,0.5,0.5,1)
        [MainTexture] _BaseMap("Albedo", 2D) = "white" {}
        _Unlit("Unlit", Float) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f LitPassVertex(appdata input)
            {
                ...
                //float distance = length(mul(UNITY_MATRIX_MV, vertex));			
				o.uv = TRANSFORM_TEX(input.uv, _BaseMap);
                //o.uv *= distance / (distance * (4));
				//o.uv *= ((distance + lerp(0, (vertex.w/2), _Distortion)) / distance);
                o.normal = vertexNormalInput.normalWS;
            ...
            half4 LitPassFragment(v2f input) : SV_Target
            {
                ...
                return half4(color, 1) * _BaseColor;
            }
            ENDHLSL
        }
    }

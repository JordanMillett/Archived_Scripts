...
public class Destructable : MonoBehaviour
{
    ...
    void Update()
    {
        if(R.isKinematic == false && Despawn == false)
        {
            Despawn = true;
            StartCoroutine(Delete(this.gameObject, 4f));

        }
            
    }

    public void Break(Vector3 Location)
    {
        ...
        /*
        Mesh originalMesh = F.sharedMesh;
        //Mesh clonedMesh = new Mesh(); 
        Mesh clonedMesh = F.mesh;
        ...
        for (int i = 0; i < clonedMesh.vertices.Length; i++)
        {
            ...
            clonedMesh.vertices[i] += new Vector3(0f,1f,0f);

        }

        F.mesh = clonedMesh;

        F.mesh.vertices = clonedMesh.vertices;
        F.mesh.triangles = clonedMesh.triangles;
			...
			if (area > MinBreakArea) {
				...
				for (int i = 0; i < sites.Length; i++) {
					...
					if (clipped.Count > 0) {
						...
						var bs = newGo.GetComponent<Destructable>();
